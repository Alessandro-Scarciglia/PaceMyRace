...
def generate_pdf_report(paces, speeds, total_time_str, params, station_time, station_ranks, stations):
    ...
    <head>
        ...
        <style>
            ...
            body {{ font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #2c3e50; margin: 0; padding: 0; line-height: 1.5; }}
            .header {{ background-color: #1f3a52; color: white; padding: 30px; margin: -20mm -15mm 25px -15mm; text-align: center; }}
            .header h1 {{ margin: 0; font-size: 24pt; font-weight: bold; letter-spacing: 1px; }}
            .header p {{ margin: 5px 0 0 0; font-size: 11pt; color: #cbd5e1; }}
            .section {{ margin-bottom: 25px; background: white; padding: 20px; border-radius: 6px; border: 1px solid #e2e8f0; }}
            h2 {{ color: #1f3a52; font-size: 14pt; margin-top: 0; margin-bottom: 15px; border-bottom: 2px solid #e2e8f0; padding-bottom: 5px; }}
            table {{ width: 100%; border-collapse: collapse; margin-bottom: 10px; }}
            th, td {{ padding: 10px 12px; text-align: left; font-size: 10pt; border-bottom: 1px solid #edf2f7; }}
            th {{ background-color: #f8fafc; color: #475569; font-weight: bold; }}
            .pace {{ font-family: 'Courier New', Courier, monospace; font-weight: bold; color: #0f172a; }}
            .summary-box {{ background-color: #f0fdf4; border: 2px solid #bbf7d0; border-radius: 6px; padding: 15px; text-align: center; margin-top: 20px; }}
            .summary-box h3 {{ margin: 0; color: #166534; font-size: 12pt; }}
            .summary-box .time {{ font-size: 28pt; font-weight: bold; color: #15803d; margin: 5px 0; }}
            .footer {{ text-align: center; font-size: 8pt; color: #94a3b8; position: absolute; bottom: 0; width: 100%; }}
        </style>
    </head>
    <body>
        <div class="header">
...
alpha = st.sidebar.slider("Race Goal Focus (α)", 0.0, 1.0, 0.5, 0.01)
st.sidebar.caption("🟢 **0.0:** Comfort | 🟡 **0.5:** Bilanciato | 🔴 **1.0:** Massima Velocità")

safety_margin = st.sidebar.slider("Margine di Sicurezza (Inizio Calmo)", 0.0, 1.0, 0.4, 0.01)
st.sidebar.caption("🟢 **0.0:** Aggressiva | 🟡 **0.5:** Cautela | 🔴 **1.0:** Molto conservativo")

stamina_decay = st.sidebar.slider("Tasso di Affaticamento / Perdita Stamina", 0.0, 1.0, 0.3, 0.01)
st.sidebar.caption("🟢 **0.0:** Élite | 🟡 **0.5:** Regolare | 🔴 **1.0:** Principiante")

heat_factor = st.sidebar.slider("Effetto Temperatura (Stress Termico)", 0.0, 1.0, 0.15, 0.01)
st.sidebar.caption("🟢 **0.0:** Fresco (<15°C) | 🟡 **0.5:** Caldo/Umido | 🔴 **1.0:** Torrido")

st.sidebar.write("---")
st.sidebar.subheader("Limiti Rigidi di Passo Corsa")

col_min_m, col_min_s = st.sidebar.columns(2)
with col_min_m: min_m = st.number_input("Passo Più Lento (Min)", 1, 20, 5)
with col_min_s: min_s = st.number_input("Passo Più Lento (Sec)", 0, 59, 0, 5)

col_max_m, col_max_s = st.sidebar.columns(2)
with col_max_m: max_m = st.number_input("Passo Più Rapido (Min)", 1, 20, 4)
with col_max_s: max_s = st.number_input("Passo Più Rapido (Sec)", 0, 59, 30, 5)
...
st.sidebar.write("---")
station_time = st.sidebar.number_input("Minuti medi spesi in ogni stazione", 0.5, 20.0, 3.0, 0.5)

# --- INSERIMENTO PARAMETRI E CALCOLO DENTRO UN FORM ---
st.header("2. Configura e Calcola la Strategia")

# Avvolgiamo le selectbox e il pulsante di calcolo in un form
with st.form("input_form"):
    st.write("Vota ogni stazione da **1 (Leggerissima)** a **5 (Brutale / Gambe di piombo)** e premi **Calcola Strategia**.")
    ...
    # Il form richiede un bottone di submit
    submit_button = st.form_submit_button(label="🚀 CALCOLA STRATEGIA DI GARA", use_container_width=True)

# L'ottimizzazione parte SOLO quando l'utente clicca sul pulsante
if submit_button or 'optimized_paces' in st.session_state:
    ...
    if result.success:
        ...
        with res_col1:
            ...
            for i in range(5):
                st.metric(
                    ...
                    value=format_pace(optimized_paces[i]), 
            ...
            # Generazione PDF (avviene solo a calcolo effettuato)
            try:
        ...
        with res_col2:
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(
                    ...
                    colorbar=dict(
                        ...
                        ticktext=["Calmo", "Intermedio", "Massimo"],
                        thickness=15
                    ),
            ...
            fig.update_layout(
                ...
                yaxis_title="Velocità Target (km/h)",
        ...
        st.error("I parametri o i limiti di passo inseriti sono in conflitto.")